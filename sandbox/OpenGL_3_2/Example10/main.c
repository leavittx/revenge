/**
 * OpenGL 3.2 with GLEW - Example 10
 *
...
/**
 * GLEW is used to retrieve all OpenGL 3.2 functions.
 */
#include <GL/glew.h>
...
/**
 * The location of the distance.
 */
GLint g_offsetLocation;
...
GLUSboolean init(GLUSvoid)
{
	...
	glusLoadTextFile("../Example10/Vertex.vs", &vertexSource);

	// Load the source of the fragment shader.
	glusLoadTextFile("../Example10/Fragment.fs", &fragmentSource);
	...
	// http://www.opengl.org/sdk/docs/man/xhtml/glGetUniformLocation.xml
	g_offsetLocation = glGetUniformLocation(g_program.program, "offset");
	...
	// http://www.opengl.org/sdk/docs/man/xhtml/glUniform.xml
	glUniform1f(g_offsetLocation, offset);

	//

	// Load the source of the vertex shader.
	glusLoadTextFile("../Example10/VertexBackground.vs", &vertexSource);

	// Load the source of the fragment shader.
	glusLoadTextFile("../Example10/FragmentBackground.fs", &fragmentSource);

	// Build and ...
	glusBuildProgram(&g_programBackground, (const GLUSchar**)&vertexSource.text, 0, (const GLUSchar**)&fragmentSource.text);
	...
	glusCreateSpheref(&background, offset*3.0f, 32);
	...
	// ToDo:
	glBindFragDataLocation(g_programBackground.program, 0, "fragColor");
